import React, { useState, useEffect, useContext } from 'react';
import { motion } from 'framer-motion';
import { GitCommit, GitPullRequest, AlertCircle, TrendingUp, Users } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import api from '../../services/api';
import { AuthContext } from '../../context/AuthContext';

const StatCard = ({ title, value, icon: Icon, colorClass, delay }) => (
  <motion.div 
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.5, delay }}
    className="glass-card p-6"
  >
    <div className="flex justify-between items-start">
      <div>
        <p className="text-textMuted text-sm font-medium mb-1">{title}</p>
        <h3 className="text-3xl font-bold">{value}</h3>
      </div>
      <div className={`p-3 rounded-lg ${colorClass}`}>
        <Icon className="w-6 h-6" />
      </div>
    </div>
  </motion.div>
);

const Overview = () => {
  const [summary, setSummary] = useState(null);
  const [chartData, setChartData] = useState([]);
  const [loading, setLoading] = useState(true);
  const { user } = useContext(AuthContext);

  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const { data } = await api.get('/analytics/summary/all');
        setSummary(data);
        
        // Fetch chart data if repos exist, else use mock placeholder
        if (data.reposAnalyzed > 0) {
          // In a real app we'd fetch actual aggregate chart data here
          // For now we just create a realistic looking sample
          setChartData([
            { name: 'Mon', commits: 12, prs: 2, issues: 3 },
            { name: 'Tue', commits: 19, prs: 4, issues: 5 },
            { name: 'Wed', commits: 15, prs: 1, issues: 2 },
            { name: 'Thu', commits: 25, prs: 5, issues: 1 },
            { name: 'Fri', commits: 22, prs: 3, issues: 4 },
            { name: 'Sat', commits: 5, prs: 0, issues: 1 },
            { name: 'Sun', commits: 8, prs: 1, issues: 0 },
          ]);
        }
      } catch (error) {
        console.error("Failed to fetch summary", error);
      } finally {
        setLoading(false);
      }
    };

    fetchDashboardData();
  }, []);

  if (loading) return <div className="animate-pulse">Loading dashboard...</div>;

  if (!user.githubUsername) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center">
        <div className="glass-card p-10 max-w-lg">
          <AlertCircle className="w-16 h-16 text-yellow-500 mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-2">Connect Your GitHub</h2>
          <p className="text-textMuted mb-6">You need to connect your GitHub account to see productivity insights.</p>
          <button className="btn-primary" onClick={() => window.location.href = '/dashboard/repos'}>
            Go to Repositories
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          title="Total Commits" 
          value={summary?.totalCommits || 0} 
          icon={GitCommit} 
          colorClass="bg-blue-500/20 text-blue-500"
          delay={0.1}
        />
        <StatCard 
          title="Pull Requests" 
          value={summary?.totalPullRequests || 0} 
          icon={GitPullRequest} 
          colorClass="bg-purple-500/20 text-purple-500"
          delay={0.2}
        />
        <StatCard 
          title="Open Issues" 
          value={summary?.totalIssues || 0} 
          icon={AlertCircle} 
          colorClass="bg-red-500/20 text-red-500"
          delay={0.3}
        />
        <StatCard 
          title="Avg Health Score" 
          value={`${summary?.averageHealthScore || 0}%`} 
          icon={TrendingUp} 
          colorClass="bg-emerald-500/20 text-emerald-500"
          delay={0.4}
        />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.5 }}
          className="glass-card p-6 lg:col-span-2 h-96"
        >
          <h3 className="text-lg font-bold mb-6">Activity Trends</h3>
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorCommits" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#3b82f6" stopOpacity={0.8}/>
                    <stop offset="95%" stopColor="#3b82f6" stopOpacity={0}/>
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} />
                <XAxis dataKey="name" stroke="#94a3b8" />
                <YAxis stroke="#94a3b8" />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1e293b', borderColor: '#334155', borderRadius: '8px' }}
                  itemStyle={{ color: '#f8fafc' }}
                />
                <Area type="monotone" dataKey="commits" stroke="#3b82f6" fillOpacity={1} fill="url(#colorCommits)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex items-center justify-center h-full text-textMuted">
              No activity data available yet. Analyze a repository first.
            </div>
          )}
        </motion.div>

        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.6 }}
          className="glass-card p-6 flex flex-col"
        >
          <h3 className="text-lg font-bold mb-4">Sprint Insights</h3>
          <div className="flex-1 flex flex-col justify-center">
            <div className="relative w-40 h-40 mx-auto mb-6">
              <svg className="w-full h-full" viewBox="0 0 100 100">
                <circle 
                  className="text-slate-700 stroke-current" 
                  strokeWidth="10" 
                  cx="50" cy="50" r="40" fill="transparent"
                ></circle>
                <circle 
                  className="text-primary stroke-current" 
                  strokeWidth="10" 
                  strokeLinecap="round" 
                  cx="50" cy="50" r="40" fill="transparent"
                  strokeDasharray={`${(summary?.averageSprintCompletion || 0) * 2.51} 251.2`}
                  strokeDashoffset="0"
                  transform="rotate(-90 50 50)"
                ></circle>
              </svg>
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                <span className="text-3xl font-bold">{summary?.averageSprintCompletion || 0}%</span>
              </div>
            </div>
            <p className="text-center text-textMuted text-sm">
              Estimated sprint completion across all tracked repositories based on closed PRs vs open issues.
            </p>
          </div>
        </motion.div>
      </div>
    </div>
  );
};

export default Overview;
